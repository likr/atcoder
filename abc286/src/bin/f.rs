...
fn main() {
    let nums = vec![4, 9, 5, 7, 11, 13, 17, 19, 23];
    // let nums = vec![2, 3, 5];
    let n = nums.len();
    let m = nums.iter().sum::<usize>();
    println!("{}", m);
    ...
    let mut modulo = vec![0; n];
    for i in 0..n {
        modulo[i] = (b[acc[i]] - acc[i] - 1) as i64;
    }
    let nums = nums.into_iter().map(|v| v as i64).collect::<Vec<_>>();
    debug!(modulo);
    let result = crt(&modulo, &nums);
    debug!(result);
    println!("{}", result.0);
}

...
fn main() {
    input! {
        ...
        m: usize,
        w: [usize; n],
        mut lv: [(usize, usize); m],
    }
    let w_max = *w.iter().max().unwrap();
    let v_min = lv.iter().map(|&(_, vi)| vi).min().unwrap();
    if w_max > v_min {
        println!("-1");
        return;
    }

    // let mut distance = vec![vec![0usize; n]; n];
    // for i in 0..n {
    //     let wi = w[i];
    //     for j in 0..n {
    //         let wj = w[j];
    ...
    // for i in 0..m {
    //     let (li, vi) = lv[i];
    //     distance.insert(vi, li);
    // }

    lv.sort_by_key(|&(_, vi)| vi);
    let mut segtree = Segtree::<Max<usize>>::new(m + 1);
    segtree.set(0, 0);
    for i in 0..m {
        let (li, _) = lv[i];
        segtree.set(i + 1, li);
    }
    let mut v = lv.iter().map(|&(_, vi)| vi).collect::<Vec<_>>();
    v.push(INF);
    debug!(v);
    ...
    loop {
        ...
        for i in 1..n {
            for j in 0..i {
                let k = v.lower_bound(&(acc[i + 1] - acc[j]));
                p[i] = max(p[i], p[j] + segtree.prod(0, k + 1));
            }
        }
        // debug!(x, d);
        result = min(result, p[n - 1]);
